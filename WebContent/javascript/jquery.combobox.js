...
//添加开始2012-02-06ygl
jQuery.fn.removeOption = function(attrName){
	var sourceSelect = jQuery(this);
	sourceSelect.find("option[index='"+attrName+"']").remove();
	if($("#dispalyControl_"+sourceSelect.attr("id")).length==0)
		sourceSelect.combobox();
}

jQuery.fn.addOption = function(opStr){
	var sourceSelect = jQuery(this);
	$(opStr).appendTo(sourceSelect);
	if($("#dispalyControl_"+sourceSelect.attr("id")).length==0)
		sourceSelect.combobox();
}
...
jQuery.fn.selectqbBoxinhylb = function(options){
	var sourceSelect = jQuery(this);
	sourceSelect.hide();
	var selData;
	var params = {
			url:"sysadmin/queryqbSelDictinhylb_dict_item.action",
			state:"",
			code:null,
			order:0,
			query:"",
...
/**
	selectBox选项控制器,
	允许通过表达式(表达式兼容JQUERY)和函数回调的方式来控制哪些选项不显示.
	@param selectboxEl selectbox容器
	@param options 选项 (选项内容: {url:''<可选>,params:'url参数',expr:'逻辑判断表达式：opt为内置变量代表 selectbox对应option<支持JQ>'})
	@param callbackFunc expr逻辑表达式回调
	
	@example 实例隐藏selectbox,OPTION 值大于10的选项.
			selectBoxOption($('#a_pxksdj'),{expr:'opt.val() > 10'}); //opt为内置变量代表option元素
*/
function selectBoxOption(selectboxEl,options,callbackFunc){
	var curOptions = {expr:'',url:'',params:{}};
		...
		if(curOptions.expr!=null && curOptions.expr != ''){
			...
			$(selectboxEl).find('option').each(function(){
				...
				if( eval(curOptions.expr)){
					...
					dp_ul = _this.find('>ul');
					dp_ul.height(dp_ul.height()-$(selectboxEl).height());
					dp_li.hide();
				}
				len++;
			});
