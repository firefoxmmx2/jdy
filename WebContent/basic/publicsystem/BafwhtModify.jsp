...
<script type="text/javascript">
	...
	function bafwht_modifyVerify(){
		...
		if (!checkControlValue("xx_qdrq","Date",null,null,null,1,"签订日期"))
			return false;
		if (!checkControlValue("xx_htyxq","Float",0.5,99.5,1,1,"合同有效期"))
		    return false;
		if  (!checkIsInteger($("#xx_htyxq").val()*12)){
			jAlert('合同有效期输入不正确，必须为整年或半年','验证信息')
			return false
		}
		...
		var intYear = new Date(strYear,strMonth-1,strDay);
		intYear.setTime(intYear.setMonth(intYear.getMonth()+parseInt($("#xx_htyxq").val()*12)));   
	    var strMonthjs = (intYear.getMonth()+1)+'';
		var strDayjs = intYear.getDate()+'';
		if(strMonthjs.length==1) strMonthjs = '0'+strMonthjs;
		if(strDayjs.length==1)  strDayjs = '0'+strDayjs;
		$("#xx_jsrq").val(intYear.getFullYear()+'-'+strMonthjs+'-'+strDayjs);
