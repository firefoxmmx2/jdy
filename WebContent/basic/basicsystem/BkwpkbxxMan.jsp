...
$(document).ready(function() {
	searchLongUrl="basicsystem/queryForExportWP_bkwpkbxx.action";
	...
	tabletitle = "";
	geteExcelHead("BkwpkbxxData");
	excelUrl = "basicsystem/getExcelCreateWP_bkwpkbxx.action";
...
function setPageList(pageno,url){	
	...
	<% if("1".equals(cmdID)) {%> 
		var bbmc="物品布控管理";
	<% } else if("2".equals(cmdID)) {%>
		var bbmc="物品撤控管理";
	<% } else if("3".equals(cmdID)) {%>
		var bbmc="物品查询管理";
	<% } %>	
	//报表请求
		//setExcel("gnlkexcel",bbmc);	
		setExcelLong("wpbkExcel",bbmc);	
	}
</script>
<html>
<body>
        ...
        <%if ("1".equals(cmdID) || "3".equals(cmdID)) {%>
			...
			<table  width="90" border="0" align="right"  cellpadding="2"  cellspacing="0">
              <tr>
                ...
                <%if (!"4".equals(cmdID))  {%>
                <td width="25%" >
						<a href="#" class="exceldcbutton" id='wpbkExcel' onclick='setExportExcel();'>导出</a>
				</td>
				<%} %>
              </tr>
            </table>
...
<div id="BkwpkbxxData" style="width:100%;">
	<table id="BkwpkbxxTable" width="100%">
	  <thead>
	    <tr>
	     	<th name="l_bkwpmc" datatype="string" sumflag="0">物品名称</th>
			<th name="l_bkwplx" datatype="string" sumflag="0">物品类型</th>
	     	<th name="l_fwdw" datatype="string" sumflag="0">布控单位</th>
			<th name="l_bkfw" datatype="string" sumflag="0">布控范围</th>
			<th name="l_bkczsj" datatype="string" sumflag="0">布控操作时间</th>
            <th name="l_bkdqsj" datatype="string" sumflag="0">布控到期时间</th>
            <%// 查询（不带操作栏）
				if ("3".equals(cmdID)) {%>
