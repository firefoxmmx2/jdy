...
var hylbdm="";
	$(document).ready(function() {
	    ...
	    if($("#p_hylbdm").val()=="A"){
	    	$("#ssqymc_title").append("旅馆名称");
	    }else{
	     	$("#m_fjh").css("display","none");
	     	$("#fjh").css("display","none");
	     	$("#ssqymc_title").append("企业名称");
	    }
	...
	function cjhfVerify(){
	    var dqsj=new Date(); 
	    var dangqsj=dqsj.getFullYear() + "-" + (dqsj.getMonth()+1) + "-" + dqsj.getDate() + " " + dqsj.getHours() + ":" + dqsj.getMinutes() + ":" + dqsj.getSeconds();
		 $("#m_dqsj").attr("value",dangqsj);
		if (!checkControlValue("m_chjsj","Datetime",null,null,null,1,"出警时间"))
			return false;
		if(!checkDateEarlier("s_pjsj","出警","m_chjsj","派警","DATETIME"))
		{
			jAlert("出警日期必须大于派警日期！",'验证信息',null,"m_chjsj");
			return false;
		}
		if(!checkDateEarlier("m_chjsj","出警","m_dqsj","当前时间","DATETIME"))
		{
			jAlert("出警日期不应该大于当前时间！",'验证信息',null,"m_chjsj");
			return false;
		}
		...
		if($("#m_cjjgdm").attr("value")=="1"){ // 已抓获
			...
			return false;
			if(!checkDateEarlier("m_chjsj","抓获","m_zhsj","出警","DATETIME"))
			{
				jAlert("抓获日期必须大于出警日期！",'验证信息',null,"m_zhsj");
				return false;
			}
			if(!checkDateEarlier("m_zhsj","抓获","m_dqsj","当前时间","DATETIME"))
			{
				jAlert("抓获日期不应该大于当前时间！",'验证信息',null,"m_zhsj");
				return false;
			}
		}else{ // 未抓获
   ...
   <tr>
    ...
    <tr>
      <td><table width="100%" id="detail2" class="detail">
        ...
        <tr>
          <td class="red"> 处警结果 </td>
          <td class="detailtd2"><input type="hidden" id="m_chjjg" />
              <select name="m_cjjgdm" id="m_cjjgdm" type="select">
              	<option></option>
              </select>
          </td>
          <td class="red" id="cjfklb"> 处警反馈类别 </td>
          <td class="detailtd2" id="cjfklbdm"><input type="hidden" id="m_cjfklb" />
              <select name="m_cjfklbdm" id="m_cjfklbdm" type="select">
                <option></option>
              </select>
          </td>
          <td class="red" id="zhsj"> 抓获时间 </td>
