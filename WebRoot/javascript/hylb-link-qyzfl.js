...
function setQyzflDic(hylbdm,qyzflId){
   ...
   function hy_lbselectBox(hylb,gwbh){
	...
	// 行业类别与企业主分类联动(全类型，带引号)
function allhylbLinkQyzfl(hylbId,qyzflId,pchyCode,onlyJK){
	var initPchyCode = "ggmk";
	if(pchyCode!=null){
		initPchyCode = pchyCode;
	}
    ...
    if(onlyJK==null){
    	 // 行业类别初期化
   		 $("#"+hylbId).selectqbBoxinhylb({code:initPchyCode});
    }else if(onlyJK=="qhy"){
    	//显示行业类别字典中的全部行业
    	 $("#"+hylbId).selectqbBoxinhylb();//行业类别
    }else{
    	// 行业类别初始化J K 当空白时为J,K
    	$("#p_hylbdm").selectqbBoxinhylb({code:"J"});
    }
...
// 根据行业类别编码设定企业主分类字典(全类型，带引号)
function setallQyzflDic(hylbdm,qyzflId){
    if("'J'"==hylbdm){//娱乐场所
        $("#"+qyzflId).selectBox({code:"dm_ylcsfl(bzh)"});
    }else if("'C'"==hylbdm){// 机修业
	    $("#"+qyzflId).selectBox({code:"dm_jxqyfl"});
 	...
 	//根据行业类别得到岗位列表
 	function allhy_gwselectBox(hylb,gwbh){
 		
 		if($('#'+hylb).val()==''||$('#'+hylb).val()==null){
 			$('#'+hylb).selectqbBoxinhylb({code:"ggmk"});
 		}
	 	if($('#'+hylb).val()=="'J'"){
	 		$('#'+gwbh).selectBox({code:"dm_cyrylb"});
	 	}else if($('#'+hylb).val()=="'E01'"){
