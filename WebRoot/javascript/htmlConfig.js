...
function checkControlValue(sControlName,sControlType,sMinValue,sMaxValue,iDecimalLength,iNullFlag,sMessage){
	...
	if (sControlType=="Integer" && sControlValue!="") {
		if (checkIsInteger(sControlValue)==false){
			jAlert(sMessage+"不是整数类型！",'验证信息',null,sControlName);
	...
	}else if (sControlType=="Float" && sControlValue!="") {//注意 当是float格式数值大于等于99999999时 会变为1.0E8以上，导致数据库无法插入，当数值过大时在bean中应改用Double类型--刘峰
		if (checkIsDouble(sControlValue)==false){
			jAlert(sMessage+"不是浮点数类型！",'验证信息',null,sControlName);
			sControlObject.get(0).select();
			return false;
		}else{
			if (sMinValue!=null){
...
// 配置页面的选填必填项的字段颜色，在对应的js中用configtable存放配置参数。
function configtdcolor(tableid,configtable){
   var table=$("#"+tableid);
   var tr=null;
   ...
   for(tr=table.find('tr'),index=0;index<length;index++)
   {
      ...
      if(tr.next().html()!=null){
         tr=tr.next();
      	   ...
      	   table.find('img').each(function(i){
     		$(this).parent().removeClass();
     		$(this).parent().css('align','center');
    });
         return;
      }    
   }
