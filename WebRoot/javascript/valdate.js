...
//根据民族汉字得到民族代码
function getMinZuCodeAndSetValue(display_name,dict_code,dmId,mcId){
	if(display_name!=null && display_name!=""){
		var params="<Params>\n";
		params+="	<Param name=\"dict_code\">"+dict_code+"</Param>\n";
		params+="	<Param name=\"display_name\">"+display_name+"</Param>\n";
		params+="</Params>\n";
	$.ajax({
		  type: "POST",
		  url: "sysadmin/queryXZQH_dict_item.action",
...
//从身份证号码中得到性别 
function getXbAll(num,xbmc,xbbm){
	var len;
	var xbNum; 
	len = num.length; 
	if (len == 15) { 
		xbNum = num.substring(14,15);
	} 
	if (len == 18) { 
		xbNum = num.substring(16,17);
	}
	if(xbNum%2==0){
	    $("#"+xbbm).val('2');
		$("#"+xbmc).val('女');
