...
var img_object='';
    ...
    function DeleteImgjdy(ob,temp)//最后一个参数用于点击取消照片时，清空提交的照片数据
    {
    ...
    function DeviceOnescanOcrjdy(ob,kuan,gao,gj,temp)//最后一个参数用于判断回调函数使用
    {
    	x=document.getElementById(ob); 
    	wi=kuan;
    	hi=gao;
    ...
    function sanTwoOcrAddValuejdy(ob,kuan,gao,gj,temp){//第三个参数用于判断回调函数
    ...
    function Devicepassportscanocrjdy(ob,kuan,gao,gj,temp)
    {
    	x=document.getElementById(ob); 
    	wi=kuan;
    	hi=gao;
    ...
    function opencardjdy(CardName,temp){//第二个参数，用于判断回调函数用到
        var IDCard;
        PickjName=CardName;
