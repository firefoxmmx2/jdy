...
$(document).ready(function() {
	...
	setParams("");
	jQuery.post("sysadmin/querySelSystem_system.action",params,getSystemBack,"json");
	daggleDiv("operatelog_detail");
}); 
	
	//封装系统下拉菜单
function getSystemBack(json){
	if(json.ldata[0] != null){
		var str = "";
		var len = json.ldata.length;
		for(var i = 0 ; i < len; i++)
			if(json.ldata[i] != null)
			str = str + "<option value='"+ json.ldata[i].systemcode +"'>" + json.ldata[i].systemname + "</option>";
		$("#sel_operatesystem").append(str);
	}
}
...
function setPageList(pageno,url){	
	...
	function getMenuDropList(){
		var sys = $("#sel_operatesystem").find("option:selected").text();
		$("#p_operatesystem").val(sys);
		if(sys == ""){
			$("#p_operatemodule").empty();
		}else{
			$("#s_systemcode").val($("#sel_operatesystem").val());
			setParams("s_");
			jQuery.post("sysadmin/querySelMenu_menu.action",params,getMenuBack,"json");
		}
	}
	
	function getMenuBack(json){
	if(json.ldata[0] != null){
		$("#p_operatemodule").empty();
		...
		for(var i = 0 ; i < len; i++)
			if(json.ldata[i] != null)
			str = str + "<option value='"+ json.ldata[i].menuname +"'>" + json.ldata[i].menuname + "</option>";
		$("#p_operatemodule").append(str);
	}
}
    ...
    <td class="tdbg">
    	<table width="100%" border="0" cellspacing="0" cellpadding="2">
				<tr>
				  ...
				  <select id="sel_operatesystem" onchange="getMenuDropList();" style="width:156;"><option></option></select>
				  <input type="hidden" id="p_operatesystem" />
				  </td>
					<td width="10%" class="pagedistd">模块</td>
				  <td width="23%" class="pagetd">
				  <input type="hidden" id="s_systemcode" />
				  <select id="p_operatemodule" style="width:156;"><option></option></select>
				  </td>
	...
	<table id="OperatelogTable" width="100%">
	  <thead>
	    <tr>       
	     	...
	     	<th name="l_operateip" datatype="string" sumflag="0">操作用户IP</th>
	     	<th name="l_operatemac" datatype="string" sumflag="0">操作用户MAC</th>
