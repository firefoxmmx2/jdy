...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#baq_qyid").attr("value",ylcsjbxx_dataid);
		$("#baq_qybaxxid").attr("value",csbaxx_dataid);
	...
	function ylcsjbxx_updatediv(json){
		$("#qybaupdate").find("span").html("").end();//隗伟修改
	...
	function csbaxx_updatediv(json){
		...
		$("#grid_baxxtjTable").find("input:checkbox").not(":checked").parent().parent().find("input:text").attr("disabled",true).addClass("readonly");
	}
	...
	function csbaxx_queryVerify(){
		...
		if (!checkControlValue("m_barq","Date",null,null,null,1,"备案日期"))
			return false;
		if(!compare_twoTime($("#m_barq").val(),'<%=currDate%>',"DATETIME")){
				jAlert("备案时间不能大于当前时间！","验证信息");
				return false;
		}
			...
			if($(this).attr("checked")==true){
				...
				if (!checkControlValue("tjrq_"+thisid,"Date",null,null,null,1,"提交日期"))
					isCanPlay = 0;
				if(!compare_twoTime($("#tjrq_"+thisid).val(),'<%=currDate%>',"DATETIME")){
					jAlert($(this).attr("name")+"提交日期不能大于当前时间！","验证信息");
					isCanPlay = 0;
				}
			}
		});
...
<div id="qybaupdate">
<input type="hidden" id="baq_qyid" value="">
<input type="hidden" id="baq_qybaxxid" value="">
