...
<script type="text/javascript">
	...
	function hylbOnchange(){
		...
		selHylb = key;
		if(selHylb=='Y01'){ //开锁业初始
			ksyInit();
		} else 	if(selHylb=='J'){ //娱乐业初始
			ylyInit();
		} else if(selHylb=='C') {
			jxyInit();
	...
	function jdyInit() { //寄递业初始化
		...
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuAdd-jdy-gzth.jsp", {}, function(){
			csflOnchange();
			frdbzjOnchange();
	...
	function ksyInit(){ //开锁业
		...
		$("#bq_ylcsfl").selectBox({code:"dm_qyzfl(ksy)",width:"118px"});
		$("#qyjbxxXiangxi").height(435);
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuAdd-ksy-gzth.jsp", {}, function(){
			csflOnchange();
			frdbzjOnchange();
			changeLableTitle('qyorcs_','企业');
			ksyYqInit(); //页签初始化
	...
	function ershoushoujiInit(){ //二手机
		...
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuAdd-esj-gzth.jsp", {}, function(){
			...
			frdbzjOnchange();
			changeLableTitle('qyorcs_','企业');
			esjYqInit(); //页签初始化 -修改于2011.07.28-添加页面添加安保信息-陈勇荣
		});
	}
	...
	function zjjgInit(){ //中介机构业
		changeLableTitle('gxdw_title','治安管辖机构');
		//$("#hqZhuFuType").css("display","none"); //当需要隐藏/显示企业主分类的时候 将此行回复注销
		$("#bq_ylcsfl").selectBox({code:"dm_qyzfl(zjjg)",width:"118px"});
		$("#qyjbxxXiangxi").height(413);
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuAdd-zjjg-gzth.jsp", {}, function(){
			csflOnchange();
			frdbzjOnchange();
			changeLableTitle('qyorcs_','企业');
		});
	...
	function ysyInit(){ //印刷业
		...
		$("#qyjbxxXiangxi").height(413);
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuAdd-ysy-gzth.jsp", {}, function(){
			csflOnchange();
			frdbzjOnchange();
			changeLableTitle('qyorcs_','企业');
			ysyYqInit();
	...
	function esjYqInit(){ //二手机 - 修改于2011.07.28-添加页面安保添加信息-陈勇荣
		showOrHideYlQitaxxbq('small_','none'); //先将除企业信息以外的页签关掉
		setYqValueAll('0'); //将除企业信息以外的页签加载状态全变成未被加载状态
		showOrHideYlQitaxxbq('small_abxx','block'); //用到安保信息页签则加载之,不需要则不用写本行和下两行
	...
	function yzhyOnloadbq(yqid){ //印章业
		...
		if(yqid=='zzcl'&&yqstate==0){
			$("#div_yzy_zhiZhiCL").empty().load("basic/publicsystem/Yqzzcl-yzy-gzth.jsp").show("slow");
		}
	}
	...
	function ksyOnloadbq(yqid){ //开锁业
	    if(yqid=='abxx'&&yqstate==0){ //保证加载一次
			...
			$("#div_ksy_baoWeiRY").empty().load("basic/publicsystem/BwryxxAddMan.jsp").show("slow"); //加载保卫人员信息
			$("#div_ksy_shiPinSB").empty().load("basic/publicsystem/BldsjksbAddMan-gzth.jsp").show("slow"); //加载视频监控设备
			$("#div_ksy_jiFangSB").empty().load("basic/publicsystem/AqjcsbMan-gzth.jsp").show("slow"); //加载技防设备信息
		}
	}
	
	function ershoushoujiOnloadbq(yqid){ //二手机 -修改于2011.07.28-添加页面安保添加信息-陈勇荣
	    if(yqid=='abxx'&&yqstate==0){ //保证加载一次
			$("#div_esj_baoAnRY").empty().load("basic/publicsystem/BaryxxMan.jsp").show("slow");//加载保安人员信息
			$("#div_esj_baoWeiRY").empty().load("basic/publicsystem/BwryxxAddMan.jsp").show("slow"); //加载保卫人员信息
			$("#div_esj_shiPinSB").empty().load("basic/publicsystem/BldsjksbAddMan-gzth.jsp").show("slow"); //加载视频监控设备
	...
	function ysyOnloadbq(yqid){ //印刷业
	    if(yqid=='abxx'&&yqstate==0){ //保证加载一次
			...
			$("#div_ysy_baoWeiRY").empty().load("basic/publicsystem/BwryxxAddMan.jsp").show("slow"); //加载保卫人员信息
			$("#div_ysy_shiPinSB").empty().load("basic/publicsystem/BldsjksbAddMan-gzth.jsp").show("slow"); //加载视频监控设备
	...
	function show_biaoqian(yqid){ //点击页签时的动作
		...
		if(setHylb='Y01'){
			...
			ershoushoujiOnloadbq(yqid);// -修改于2011.07.28-添加页面安保添加信息-陈勇荣
		} else if(selHylb=='E04') { //废旧金属
			fjjsOnloadbq(yqid);
		} else if(selHylb=='F01') { //出租车
			//czcOnloadbq(yqid);
