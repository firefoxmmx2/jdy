...
<script type="text/javascript">
	...
	function jdyInit() { //寄递业初始化
		changeLableTitle('gxdw_title','管辖单位');
		$("#bq_ylcsfl").selectBox({code:"dm_qyzfl(jdy)",width:"118px"});
		$("#qyjbxxXiangxi").height(435);
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuModify-jdy-gzth.jsp", {}, function(){
	...
	function ksyInit(){ //开锁业
		changeLableTitle('gxdw_title','管辖单位');
		//$("#hqZhuFuType").css("display","none"); //当需要隐藏/显示企业主分类的时候 将此行回复注销
		$("#bq_ylcsfl").selectBox({code:"dm_qyzfl(ksy)",width:"118px"}); //主分类
		$("#qyjbxxXiangxi").height(455);
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuModify-ksy-gzth.jsp", {}, function(){
			qyjbxxXiangxi_updatediv();
	...
	function ershoushoujiInit(){ //二手机
		...
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuModify-esj-gzth.jsp", {}, function(){
			...
			frdbzjOnchange(); //法人代表证件类别隐藏域
			changeLableTitle('qyorcs_','企业');
			esjYqInit();// -修改于2011.07.28-添加页面安保修改信息-陈勇荣
		});
	}
	...
	function zjjgyInit(){ //中介机构
		changeLableTitle('gxdw_title','管辖单位');
		//$("#hqZhuFuType").css("display","none"); //当需要隐藏/显示企业主分类的时候 将此行回复注销
		$("#bq_ylcsfl").selectBox({code:"dm_qyzfl(zjjg)",width:"118px"});
		$("#qyjbxxXiangxi").height(432);
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuModify-zjjg-gzth.jsp", {}, function(){
			qyjbxxXiangxi_updatediv();
			hylbOnchange(); //行业类别隐藏域
			csflOnchange();
			frdbzjOnchange();
		...
		changeLableTitle('gxdw_title','管辖单位');
		//$("#hqZhuFuType").css("display","none"); //当需要隐藏/显示企业主分类的时候 将此行回复注销
		$("#bq_ylcsfl").selectBox({code:"dm_qyzfl(ysy)",width:"118px"});
		$("#qyjbxxXiangxi").height(432);
		$("#QyJibenxiangmu").load("basic/publicsystem/QyjbxxJibenXiangMuModify-ysy-gzth.jsp", {}, function(){
			qyjbxxXiangxi_updatediv();
			hylbOnchange(); //行业类别隐藏域
			csflOnchange();
			frdbzjOnchange();
	...
	function esjYqInit(){ //二手机 - 修改于2011.07.28-添加页面安保修改信息-陈勇荣
		showOrHideYlQitaxxbq('small_','none'); //先将除企业信息以外的页签关掉
		showOrHideYlQitaxxbq('small_abxx','block'); //用到安保信息页签则加载之,不需要则不用写本行和下两行
	...
	function yzhyOnloadbq(yqid){ //印章业
		...
		if(yqid=='zzcl'&&yqstate==0){
			$("#div_yzy_zhiZhiCL").empty().load("basic/publicsystem/YqzzclModify-yzy-gzth.jsp").show("slow");//加载技防设备信息
		}
	}
	
	//---不同行业页签点击事件初始化(根据需求,不同行业每个页签加载的内容可能不同：在这里做)
	function ksyOnloadbq(yqid){ //开锁业
	    if(yqid=='abxx'&&yqstate==0){ //保证加载一次
	    	//alert($("#div_ksy_baoAnRY").html())
			$("#div_ksy_baoAnRY").empty().load("basic/publicsystem/BaryxxMan.jsp").show("slow");//加载保安人员信息
			$("#div_ksy_baoWeiRY").empty().load("basic/publicsystem/BwryxxModifyMan.jsp").show("slow"); //加载保卫人员信息
		$("#div_ksy_shiPinSB").empty().load("basic/publicsystem/BldsjksbModifyMan-gzth.jsp").show("slow"); //加载视频监控设备
	...
	function ddyOnloadbq(yqid){ //典当业
		...
		if(yqid=='zzcl'&&yqstate==0){
			$("#div_ddy_zhiZhiCL").empty().load("basic/publicsystem/ZzclxxxModify.jsp").show("slow");//加载技防设备信息
		}
	}
	...
	function ershoushoujiOnloadbq(yqid){ //二手机 - 修改于2011.07.28-添加页面安保修改信息-陈勇荣
	    if(yqid=='abxx'&&yqstate==0){ //保证加载一次
			$("#div_esj_baoAnRY").empty().load("basic/publicsystem/BaryxxMan.jsp").show("slow");//加载保安人员信息
			$("#div_esj_baoWeiRY").empty().load("basic/publicsystem/BwryxxModifyMan.jsp").show("slow"); //加载保卫人员信息
			$("#div_esj_shiPinSB").empty().load("basic/publicsystem/BldsjksbModifyMan-gzth.jsp").show("slow"); //加载视频监控设备
	...
	function ysyOnloadbq(yqid){ //印刷业
	    if(yqid=='abxx'&&yqstate==0){ //保证加载一次
			...
			$("#div_ysy_baoWeiRY").empty().load("basic/publicsystem/BwryxxModifyMan.jsp").show("slow"); //加载保卫人员信息
			$("#div_ysy_shiPinSB").empty().load("basic/publicsystem/BldsjksbModifyMan-gzth.jsp").show("slow"); //加载视频监控设备
	...
	function show_biaoqian(yqid){ //点击页签时的动作
		...
		if(selHylb=='J'){ //娱乐业初始
			...
			ershoushoujiOnloadbq(yqid);// -修改于2011.07.28-添加页面安保修改信息-陈勇荣
		} else if(selHylb=='E04') { //废旧金属
			fjjsOnloadbq(yqid);
		} else if(selHylb=='F01') { //出租车
			//czcOnloadbq(yqid);
