...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#detail_csfltj").hide();
		daggleDiv("detail_csfltj");
		pageUrl="publicsystem/queryCsfltjlist_cxtj.action";
		divnid="tabledata_csfltj";
		tableid="table_csfltj";
		loadPage(divnid);
		ksd_csxxtjInit();
		initTJDate();
    ...
    function createDataTable(){
		...
		var ganjCol = "<td id='l_gxdw' width='20%' rowspan='2' class='grid-header-bg1'>省厅</td>";
    	jb="st"
        $("#gajgAll").find("input[type='checkbox']").each(function(){
			if($(this).attr("checked")==true){
				if($(this).attr("dataKey")=='dsjgdm'){
					jb="ds";
					ganjCol = "<td id='l_gxdw' width='20%' rowspan='2' class='grid-header-bg1'>市级公安机关</td>";
				}
				if($(this).attr("dataKey")=='fxjdm'){
					jb="fxj";
					ganjCol = "<td id='l_gxdw' width='20%' rowspan='2' class='grid-header-bg1'>分县局公安机关</td>";
				}
				if($(this).attr("dataKey")=='gxdwdm'){
					jb="gxdw";
					ganjCol = "<td id='l_gxdw' width='20%' rowspan='2' class='grid-header-bg1'>科所队</td>";
				}
			}
		});
		var result = "<table id='"+tableid+"' width='100%'><thead><tr id='tr1'>";
			result += ganjCol;
			result += "<td id='l_zqnxz'  width='81%' colspan='10' class='grid-header-bg1'>周期内新增</td>";
			result += "</tr><tr id='tr2'>";
	     	result += "<td  class='grid-header-bg1' >场所数</td>";
	     	result += "<td  class='grid-header-bg1' >装机数</td>";
	     	result += "<td  class='grid-header-bg1' >装机率</td>";
	     	result += "<td  class='grid-header-bg1' >歌舞娱乐</td>";
	     	result += "<td  class='grid-header-bg1' >游艺场所</td>";
	        result += "<td  class='grid-header-bg1' >其他娱乐场所</td>";
	     	result += "<td  class='grid-header-bg1' >桑拿洗浴按摩</td>";
	        result += "<td  class='grid-header-bg1' >美容美发足浴</td>";
	        result += "<td  class='grid-header-bg1' >其他服务场所</td>";
	        result += "<td  class='grid-header-bg1' >公共场所</td>";
			result += "</tr></thead><tbody></tbody></table>";
			$("#tabledata_csfltj").html(result);
		loadPage(divnid);
    }
	function setPageList(pageno,url){
		...
		$("#l_gxdw").attr('style',"text-align:center;");
		$("#l_zqnxz").attr('style',"text-align:center;");
		$("#tr1").addClass("grid-header-tr");
		$("#tr2").addClass("grid-header-tr");
	}
	...
	function setXxQuery(id){
	 	...
	 	detailid="detail_csfltj";
     	setWidth(detailid,"800");
     	$('#'+detailid).css('left','80').css('top','40');
	 	setUrl(detailid,"basic/publicsystem/Csfltjxx.jsp");
	}
    ...
    <td valign="top" class="tdbg">
		...
		<tr >
			...
			<td width="23%"  class="pagetd">
				...
				<td><a href="#" class="exceldcbutton" onClick="expExcel('table_csfltj',11,2)" id="csfltjExcel">导出</a></td>
				</tr>
				</table>
			</td>
		</tr>
...
<div id="detail_csfltj" class="page-layout" src="#" style="top:20px; left:160px; width:1000px;"></div>
<div id="tabledata_csfltj" style="width:100%;">
	<table id="table_csfltj" width="100%">
	  <thead>
        <tr id="tr1">
	         <td id="l_gxdw" width="20%" rowspan="2" class="grid-header-bg1">管辖单位</td>
	         <td id="l_zqnxz"  width="81%" colspan="10" class="grid-header-bg1">周期内新增</td>
        </tr>
	    <tr id="tr2">
