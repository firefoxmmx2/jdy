...
<script type="text/javascript">
	...
	function qyjbxx_setList(pageno,url){	
		...
		if (url==null || url=="undefined"){
			url="publicsystem/querySkscmxList_wscqy.action";
		}
		return url;
	}
				...
				onRowSelect: function(tr, selected){
					...
					if(sFlag=="true"){
						if ($(tr).attr('_selected')=="true"){
							var qybm=$(tr).find("td:nth(2)").text();
							var tjrq=$(tr).find("td:nth(0)").text();
							setcsskjlQuery(qybm,tjrq);
						}
					}else{
						sFlag="true";
					}
	...
	function setcsskjlQuery(qybm,tjrq){
		 qyjbxx_dataid=qybm+"__"+tjrq;
	     detailid="csskjl_detail";
		 setWidth(detailid,'990');
	     $('#'+detailid).css('top','30');
	     $('#'+detailid).css('left','20');
		 var url="basic/icksystem/skwscCsxxMan-drskjl.jsp";
	     setUrl(detailid,url);
	}
	
	// 导出Excel
	function setExportExcel_sksc(){ 
	    if(manVerify()){
	...
	*/
	function setSearchLong_sksc(){
		setParams("x_");
		jQuery.post("publicsystem/querySkscmxListExcelTemp_wscqy.action",params,searchLongBack_sksc,"json");
	}
	//导出前对应setSearchLong()的回调方法  由于执行查询时候有延迟，故将导出放入回调函数

	function searchLongBack_sksc(json){  
		//报表标题
		var bbmc="刷卡统计未上传场所";
		//报表请求
		setExcelLong_sksc("skscExcel",bbmc);	
	}
	 ...
	 */
	function setExcelLong_sksc(excelid,bbmc){
		var surl="publicsystem/querySkscmxListExcel_wscqy.action"+"?tabletitle="+tabletitle+"&bbmc="+bbmc;
		surl=encodeURI(surl);
		location.href = surl;
	}
</script>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center">
    <tr>
      <td align="left" class="title1">刷卡统计上传场所信息</td>
...
<div id="csskjl_detail" class="page-layout" src="#" style="z-index:2000; top:-50px; left:0px; display:none;"></div>
<table  width="90%"  border="0" cellpadding="0"  cellspacing="0">
    <tr>
      <td align="right" ><a href="#" class="exceldcbutton" id="skscExcel" onClick="setExportExcel_sksc();">导出</a></td>
    </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr><td height="3"></td></tr>
