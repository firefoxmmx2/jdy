...
$(document).ready(function() {
	...
	//*****导出Excel ***********************************
	tabletitle = "";  
	excelUrl="publicsystem/queryRcjcxxListExcelTemp_publicrcjc.action";
	searchLongUrl="publicsystem/queryRcjcxxListExcel_publicrcjc.action";  //超长参数后台Action对应url
	//*********导出至**************************************
...
//	}
	...
	$("#p_qymc").click(function(){
	 	dataid="";
	    ghylbbz=$('#p_hylbdmall').val();
	 	getTy_item("p_qymc","","p_qyid","","",ghylbbz);
	 });
	setHyxx();
}); 
...
function manVerify(){
	if(rcjcNum!=0){
		...
		if (!checkControlValue("p_jcrybh","String",1,20,null,0,"检查人警号"))
			return false;
		if (!checkControlValue("p_jcrxm","String",1,30,null,0,"检查人姓名"))
			return false;
		if (!checkControlValue("p_jcrzw","String",1,100,null,0,"检查人职务"))
			return false;
	}
	return true;
}
...
function updatehylb(json){
	$("#p_hylbdmall").setValue(setNull(json.LQyjbxx[0].hylbdm));
	//$("#p_hylbdmall").setAttr("readonly",true);
}
...
/*function GetALine(bmid,mcid){   
 ...
 //设定行业信息表示
    function setHyxx(){
        var hylb=$("#p_hylbdmall").val();
        if(hylb !=null){
			if(hylb.indexOf("J")!=-1 || hylb.indexOf("K")!=-1){
				$('#td_qymc').text('场所名称');
				$('#td_qybm').text('场所备案编号');
		...
		shouci = false;
    		...
    		//导出Excel
	function setRcjcExportExcel(){
		if(manVerify()){
			var result = "<tr>";
		  	result += "<th name='l_jcrq'  datatype='String' sumflag='0'>检查时间</th>";
			result += "<th name='l_lsdw'  datatype='String' sumflag='0'>检查机构</th>";
			result += "<th name='l_jcrxm'  datatype='String' sumflag='0'>检查人</th>";
	     	result += "<th name='l_jcrybh'  datatype='String' sumflag='0'>检查人警号</th>";
	     	result += "<th name='l_jcrzw'  datatype='String' sumflag='0'>检查人职务</th>";
			...
			<td class="tdbg">
				<table width="100%" border="0" cellspacing="0" cellpadding="2">
					...
					<tr>
						...
						<td width="7%" class="pagedistd">检查人警号</td>
						<td width="17%" class="pagetd"><input type="text" class="inputstyle" id="p_jcrybh" /></td>
						<td width="7%" class="pagedistd">检查人姓名</td>
						<td width="17%" class="pagetd"><input type="text" class="inputstyle" id="p_jcrxm" /></td>
					</tr>
					<tr>
						<td width="7%" class="pagedistd">检查人职务</td>
						<td width="17%" class="pagetd">
							<input type="text" class="inputstyle" id="p_jcrzw"/>
						</td>
						<td class="pagetd" colspan="4">
							<table border="0" align="right" cellpadding="2" cellspacing="0">
								<tr>
									...
									<td>
										<a href="#" class="exceldcbutton" onClick="setRcjcExportExcel();" id="rcjcExcel">导出</a>
									</td>
								</tr>
							</table>
						</td>
