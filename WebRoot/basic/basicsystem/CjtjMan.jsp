...
<%
	...
	String deptCodeTrim = StringUtil.trimEven0(deptCode);
	String deptlevel = String.valueOf(user.getDepartment().getDepartlevel());
	String isleaf = user.getDepartment().getIsleaf();
 %>
...
$(document).ready(function() {
	...
	$("#p_hylbdmall").selectBoxinhylb({code:"dm_hylb"});// 行业类别选择框
	if($("#p_hylbdmall").find("option").length<=2){
		$("#p_hylbdmall").setValue($("#p_hylbdmall").find("option:nth(1)").val());
...
function setPageList(pageno,url){
	...
	$("#t_hylbdmall").val($("#p_hylbdmall").val());
...
<input type="hidden" id="p_isleaf" value="<%=isleaf %>">
<input type="hidden" id="t_state">
<input type="hidden" id="t_gxdwlevel">
<input type="hidden" id="t_hylbdmall">
    	...
    	<table width="100%" border="0" cellspacing="0" cellpadding="2">
				<tr>
					...
					<td width="15%" class="pagetd">
						<input type="hidden" id="p_gxdwbm" value="<%=deptCodeTrim %>">
						<input type="hidden" id="p_gxdwlevel" value="<%=deptlevel %>">
						<input type="hidden" id="p_gxdwbm_full" value="<%=deptCode %>">
						<input type="text" class="inputstyle" id="s_gxdwmc" value="<%=deptName %>">
					</td>
					<td width="8%" class="pagedistd">行业类别</td>
					<td width="17%" class="pagetd">
						<select type="select" id="p_hylbdmall">
						</select>
					</td>
				</tr>
				<tr>
