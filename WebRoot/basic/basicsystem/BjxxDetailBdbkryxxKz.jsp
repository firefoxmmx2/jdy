...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		setParams("m_");
		// 提交到BjxxbAction中,调用其中的query方法
		jQuery.post("basicsystem/query_bkry_kz.action",params,bdbkkzdiv,"json"); 
		//bkryZp = 布控人员照片
	...
	function bdbkkzdiv (json) { 
		...
		$("#bjsj_bk").append(setNull(json.LBkry_kz[0].bjsj));	//从bjxxb中取
		$("#bjlx_bk").append(setNull(json.LBkry_kz[0].bjlx));	//从bjxxb中取
		...
		if(bkrylxdm=="1"){
			$("#xm_bk").append(setNull(json.LBkry_kz[0].zwm));
		   	...
		   	$("#zjhmtd").text("证件号码");
		   	$("#ywx_bk").append(setNull(json.LBkry_kz[0].ywx));	//英文姓
		   	$("#ywm_bk").append(setNull(json.LBkry_kz[0].ywm));	//英文名
		   	$("#gwtr").css("display","block");
		   	$("#gwtr1").css("display","none");
		}else{
		  	$("#xm_bk").append(setNull(json.LBkry_kz[0].xm));
		  	$("#gmsfhm_bk").append(setNull(json.LBkry_kz[0].gmsfhm));
		  	$("#zjhmtd").text("身份证号码");
		  	$("#gwtr").css("display","none"); /////隐藏外文姓名
		}
