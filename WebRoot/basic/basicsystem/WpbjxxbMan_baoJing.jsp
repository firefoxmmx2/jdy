...
$(document).ready(function() {
	...
	detailWidth="800";
	divnexpid="WpbjxxbExpData";
	
	// Excel表头
	tabletitle = "";
	//调用htmlConfig.js中getExcelHead方法，得到表头。
	geteExcelHead(divnexpid);
	//alert(tabletitle);
	// ExcelURL
	...
	// 警情是否有效选择框
	$("#p_sfyxbjdm1").selectBox({code:"dm_bez"}); 
	// 报警类型选择框
...
function cjhf(id){
	$("#current_recordBjlx").attr("value",$("#WpbjxxbData").find("tr[id='"+id+"']").find("td").eq(4).text()); //保存本记录的报警类型
	$("#m_bjxxid2").attr("value",id); // 保存本记录的报警信息id
	$("#cjjg").attr("value",$("#WpbjxxbData").find("tr[id='"+id+"']").find("td").eq(7).text()); //保存本记录的报警类型
	sFlag="false";
	setWidth(detailid,modWidth);
//	hylbdmzjz=$('#p_hylbdm').val();
//	alert(hylbdmzjz);
	$("#wpbjxxb_detail").empty();
...
function setExportExcel(){
	//alert(tabletitle);	
  	if(daochuNum==1){
		if (manVerify()){
			//传全部参数将查询结果放入json，对应后台Action方法中将结果集放入session，用于处理超长参数的数据导出
  			setSearchLong();
  			//alert(tabletitle);
  		}
  	}else{
...
<html>
<body>
	<input type="hidden" id="cjjg"> <!-- 出警情况，用于修改回复时用 -->
...
<table width="100%" cellpadding="0" cellspacing="0"  class="tableborder">
  <tr>
    <td class="queryfont">物品报警信息查询</td>
  </tr>
  <tr>
				<td class="tdbg">
					<table width="100%" border="0" cellspacing="0" cellpadding="2">
						...
						<tr>
							...
							<td  class="pagetd">
								<select type="select" id="p_sfyxbjdm1">
									<option></option>
								</select>
							</td>
							<td colspan="5">
								<table width="150" border="0" align="right" cellpadding="2"
	                                ...
	                                <tr>
										...
										<td width="25%" >
											<a href="#" class="exceldcbutton" id='bjxxExcel' 
...
<div id="WpbjxxbExpData" style="width:100%; display:none">
	<table id="WpbjxxbExpTable" width="100%">
		<thead>
			<tr>
...
<!--Excel表头 
 	name domain的属性以l_开头；
 	datatype 数据类型 （字符 string,数字 number,日期 date）;
 	sumflag 0不合计，1合计
 -->
