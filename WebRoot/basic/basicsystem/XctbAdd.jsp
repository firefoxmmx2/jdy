...
<script type="text/javascript">
	...
	//公安机关选中 
	 $("#a_gajg").bind('click',function(){   
	    if($("#a_gajg").attr("checked")){        
		    gajgcheck=1;
		    $("#hylb").hide();
		}else{
		    gajgcheck=0;
    ...
    //添加验证 
	function addVerify(){
		if(document.getElementById("a_sxhylbmc").value==""&&gajgcheck==0){
		    jAlert("请先选择行业类别或者公安机关！","验证信息",null,null);
		    return false;
		    }
		if(!checkControlValue("a_jsdwmc","String",1,10000,null,1,"接收单位名称"))
		    return false;
	...
	$("#a_jsdwmc").click(function(){
	    ...
	    var hylbdmall = hylbbm;
	    $("#a_jsdwmc").val("");
	    $("#a_jsdwbm").val("");
	    if(hylbdmall==""&&gajgcheck==0){
		    jAlert("请先选择行业类别或者公安机关！","验证信息",null,null);
		  }
	    if(gajgcheck==1)
	    {
	      jQuery.post("basicsystem/clearGajg_xctb.action",null,null,"json");
	      $("#a_jsdwmc").val("");
		  $("#a_jsdwbm").val("");  
		  getXctbgxdwTree("a_jsdwmc","a_jsdwbm",null,null,null,null,null,null);
	    }else if(hylbdmall!=""&&gajgcheck==0){
	...
	function initxctbAdd(json){
	 ...
	 //$("#tdhylb").find("input:checkbox").click();           //添加页面默认选中行业类别 
	}
	...
	function openfile(){
	   ...
	   if(hylbdm==""&&gajgcheck==0){
	     jAlert("请选择行业类别或者公安机关",'提示信息');
	     return false;
	   }
...
<input type="hidden" id="a_departcode" value=<%=deptCode%> >
...
<table width="100%" cellpadding="2" cellspacing="0" id="add">
  ...
  <tr id="gajg">
    <td colspan="6" valign="top">
    	<table width="100%"  cellpadding="0" cellspacing="0"  class="tableborder">
      		<tr>
       		 <td class="queryfont">公安机关</td> 
      		</tr>
      		<tr>
       			<td class="tdbg" align="left" valign="top">
       				<table width="100%" border="0" cellspacing="0" cellpadding="2">
            			<tr>
              				<td><input type="checkbox" id="a_gajg" class="checkbox"/>公安机关</td>            
            			</tr>
        			</table>
  				 </td>
  			</tr> 
  ...
  <tr>
    <td class="red" valign="top">通知通报<br>内&nbsp;容</td>
