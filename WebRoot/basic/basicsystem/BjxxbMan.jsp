...
$("#p_hylbdmall").change(function(){// 下面中的方法移动到hylbdmallchange方法中。刘峰修改行业类别切换时业务类别问题时修改。
		hylbdmallchange();
	});
function hylbdmallchange(){
	if($("#p_hylbdmall").attr("value")=="'A'"){
			$("#ywlbname").show();
		  	$("#ywlbbky").hide();
			$("#p_ywlbdm").selectBox({code:"dm_ywlb"}); // 业务类别选择框
		}else if($("#p_hylbdmall").attr("value")=="'N'"){
			$("#ywlbname").show();
		  	$("#ywlbbky").hide();
			$("#p_ywlbdm").selectBox({code:"dm_nbdw_hyfl"}); // 业务类别选择框	
		}else{
...
function querybj(){
  // if (!checkControlValue("p_hylbdm","Select",1,100,null,1,"行业类别"))   //江苏需求变更 20110505
//			return false;
	if($("#s_gxdwmc").val()==""){
		$("#p_gxdwbm").val("");
		$("#p_gxdwlevel").val("");
				...
				<td class="tdbg">
					<table width="100%" border="0" cellspacing="0" cellpadding="2">
						<tr>
							...
							<td width="16%" class="pagetd" id="ywlbname">
								<select type="select" id="p_ywlbdm">
									<option></option>
								</select>
							</td>
							<td  class="pagetd" id="ywlbbky">
