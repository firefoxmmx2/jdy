...
<%
	...
	else
	{
		...
		mobilephone = user.getMobilephone() == null ? "" : user.getMobilephone();//改动
	}
 %>
<script type="text/javascript">
	...
	function setReadonly() {
		if($("#modify1").attr("checked") == false) {
			$("#o_password").attr("value","");
			$("#o_passwordNew").attr("value","");
			$("#o_passwordNew2").attr("value","");
			$("#td1").hide();
			$("#td2").hide();
			$("#td3").hide();
			$("#td4").hide();
	...
	function setUpdatePassword(){ 
			...
			if($("#modify1").attr("checked") == true) {
				...
				sUrl="sysadmin/updatePassword_user.action";
			} else {
				sUrl="sysadmin/updatePassword_user.action?mark=mark";//如果不修改密码，提交一个标记
			}
			if(!checkControlValue("o_mobilephone","String",1,400,null,0,"手机号码"))
					return false;
			$("#a_username").val($("#o_username").val());
			$("#a_useraccount").val($("#o_useraccount").val());
			$("#a_mobilephone").val($("#o_mobilephone").val());
          setParams("a_");
          ;
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center">
    <tr>
      <td align="left" class="title1">个人信息修改</td>
...
<tr height="12"></tr>
  ...
  <tr>
  	<td><input type="checkbox" id="modify1" onclick="setReadonly()"></td>
  	<td align="left">密码修改</td>
  </tr>
	<tr>
	  <td class="red" id="td1">原口令</td>
	  <td class="detailtd" id="td2"><input type="password" id="o_password" class="inputstyle" /><input type="hidden" id="a_password"/></td>
  </tr>
	<tr>
	  <td class="red" id="td3">新口令</td>
	  <td class="detailtd" id="td4"><input type="password" id="o_passwordNew" class="inputstyle" /><input type="hidden" id="a_passwordNew"/></td>
  </tr>
	<tr>
	  <td class="red" id="td5">重复新口令</td>
	  <td class="detailtd" id="td6"><input type="password" id="o_passwordNew2" class="inputstyle" /><input type="hidden" id="a_passwordNew2"/></td>
  </tr>
  <tr>
	  <td class="distd">手机号码</td>
	  <td class="detailtd"><input type="text" id="o_mobilephone" class="inputstyle" onkeyup="value=value.replace(/[^\d ,^\- ^(,)]/g,'')" value="<%= mobilephone %>"/><input type="hidden" id="a_mobilephone" /></td>
  </tr>
