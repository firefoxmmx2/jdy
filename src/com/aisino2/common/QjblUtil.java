...
public class QjblUtil {
	...
	public static String findstationid(javax.servlet.http.HttpSession session) {
		// javax.servlet.http.HttpSession session =
		// ServletActionContext.getRequest().getSession();
		ctx = ContextLoader.getCurrentWebApplicationContext();
		com.aisino2.sbsqzagl.service.ISbsqService sbsqService = (com.aisino2.sbsqzagl.service.ISbsqService ) ctx.getBean("sbsqService");
		User user = (User) session.getAttribute(Constants.userKey);
		String sbsqbh = "0";
		if (user != null) { 
			com.aisino2.sbsqzagl.domain.Sbsq setsbsq = new com.aisino2.sbsqzagl.domain.Sbsq();
			if(user.getSsdwbm()==null||"".equals(user.getSsdwbm())){
				user.setSsdwbm(user.getUseraccount());
			}
			setsbsq.setStationid(user.getSsdwbm());
			setsbsq = sbsqService.getsbsq(setsbsq);
			if (setsbsq != null) {
				sbsqbh = setsbsq.getSbsqbh();
			}
		}
		return sbsqbh;
	}
