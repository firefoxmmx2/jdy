...
public class LoginAction extends PageAction{
		    ...
		    if(setUser==null){
			    ...
			    if(b.after(a)){
			    	...
			    	if("0".equals(setUser.getIsvalid()))
					{
						...
						String zsyxrq = sdf.format(a);
						long difday = difDay(zsyxrq);
						if(difday<31){
							this.getRequest().getSession().setAttribute("shouquanEndDate", "证书有效截止日期:"+zsyxrq);
						}
						//对登录用户进行验证判断是否有权限登录页面
						if(loginJudge(setUser)){
							//通过验证
	...
	public String  loginGaCA() {
		try {
			User setUser = new User();
			setUser = (User)this.setClass(setUser, null);
			String way = setUser.getWay();
			baojingflag = setUser.getBaojingflag();
			setUser = (User) userService.checkGaCAUser(setUser);
			if (setUser == null) {
				this.result="登录失败：输入的用户名或密码错误！";
			} 
			else if("0".equals(setUser.getIsvalid()))
	...
	private User loadInformation(String way,String bjflag,User getuser ,boolean ca) throws Exception{
		...
		if(usertype!=null&&!usertype.equals("")){
			...
			for(int i=0;i<utype.length;i++){
				if("15".equals(utype[i]))
					whdflag = 1;
			}
		}
		
		if(whdflag==1){
			userDepartment.setUserid(getuser.getUserid());
			userDepartment=departmentService.getDepartment_ty(userDepartment);
		}else{
		...
		String loginip = "";
		String loginMac = "";//Mac地址
		loginip = ipMac.getIpAddr(request);
		//获得Mac地址
		loginMac = ipMac.getMACAddress(loginip);
		
		
		//存入登录日志
		Loginlog loginlog = new Loginlog();
		loginlog = loadLoginlog(loginip,loginMac,getuser);
		//登录日志放入登录用户
		...
		session.setAttribute("LOGIN_MAC", loginMac);
		...
		try {
			...
			for(int i=0;i<arrUserType.length;i++){
				...
				if(("12".equals(userType) || "142".equals(userType)) && !nwwbz.equals(nwwbz_cjd)){
					...
					if (!"113".equals(userType)&&!"112".equals(userType)&&!"53".equals(userType)) {
						//剧毒化学品企业以及人员不校验装机营业状态@2011-07-20 何靖					
						//各行业企业端用户、员工用户只有营业且装机开业的企业,才能登陆上传企业数据
				    	//userType=53属于印章业使用单位用户，该用户不存在qyjbxx表内，所以不需要校验@2011-12-28陈勇荣
					String qybm = queryQybm(getuser).getSsdwbm();
					if(qybm != null && !"".equals(qybm) ){
						//装机开业为‘1’，娱乐业营业为‘11’,其他行业营业为‘1’
						User getQyztByUser = userService.getQyztByQybm(qybm);
						if(!("1".equals(getQyztByUser.getYyztdm()) || "11".equals(getQyztByUser.getYyztdm())) || !"1".equals(getQyztByUser.getZjztdm())){
							this.result = "登录失败：营业且装机开业的用户才能登陆！";
							returnback = false;
						}
					}
				    }
	...
	/**
	 * 存储日志信息
	 * @param String loginip,String loginmac,User getuser
	 * @return Loginlog
	 */
	//
	private Loginlog loadLoginlog(String loginip,String loginmac,User getuser){
		Loginlog loginlog = new Loginlog();
		...
		loginlog.setLoginmac(loginmac);//登陆日志存入Mac地址
		...
		if("1".equals(qykfkg)&&!isContentsUserType(mapTg, userType)&&isContentsUserType(mapUse, userType)){ //企业端用户
			try {
				...
				if("1".equals(isright)){
					...
					if(difday<31){
						session.setAttribute("shouquanEndDate", "授权截止日期:"+endDate+" "+QjblUtil.queryQjblVal("qykf-xxtxbc"));
					}
				}else{
					result = "授权无效";
					res = false;
		...
		for(int i=0;i<utype.length;i++){
			if("112".equals(utype[i])){
				...
				break;
					...
					||"33".equals(utype[i])
					||"43".equals(utype[i])
					||"52".equals(utype[i])//印章制作单位 liufei change
					||"53".equals(utype[i])//印章使用单位
					||"63".equals(utype[i])
					||"73".equals(utype[i])
					||"83".equals(utype[i])
					||"93".equals(utype[i])
		...
		public String  galoginjy() {
			...
			try {
				...
				else if("0".equals(setUser.getIsvalid()))
				{
					...
					this.result="登陆用户不为公安用户！";
					String[] utype = setUser.getUsertype().split(",");
					for(int i=0;i<utype.length;i++){
