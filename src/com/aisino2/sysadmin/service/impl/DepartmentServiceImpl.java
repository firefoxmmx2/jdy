...
public class DepartmentServiceImpl extends BaseService implements IDepartmentService {
	...
	public Department getDepartment_ty(Department department){
		// TODO: implement
		StringBuffer sqlStmt = new StringBuffer();
		sqlStmt.append(" select a.departid departid,a.departcode departcode,a.departname departname, ");
		sqlStmt.append(" a.departlevel departlevel,a.parentdepartid parentdepartid,a.departfullcode departfullcode, ");
		sqlStmt.append(" a.nodeorder nodeorder,a.isleaf isleaf,a.departsimplepin departsimplepin, ");
		sqlStmt.append(" a.departallpin departallpin,a.departbrevitycode departbrevitycode,a.departmentType, ");
		sqlStmt.append(" nvl(b.departname,'顶级') parentdepartname,(case when ");
		sqlStmt.append(" (select count(1) ");
		sqlStmt.append(" from t_user t1 ");
		sqlStmt.append(" where t1.departid in ");
		sqlStmt.append(" (select departid ");
		sqlStmt.append(" from t_department_ty t2 ");
		sqlStmt.append(" where t2.departfullcode like trim(a.departfullcode) || '%' ");
		sqlStmt.append(" )) + ");
		sqlStmt.append(" (select count(1) ");
		sqlStmt.append(" from t_role t1 ");
		sqlStmt.append(" where t1.departid in ");
		sqlStmt.append(" (select departid ");
		sqlStmt.append(" from t_department_ty t2 ");
		sqlStmt.append(" where t2.departfullcode like trim(a.departfullcode) || '%' ");
		sqlStmt.append(" )) >0 then 1 else 0 end) child_count ");
		sqlStmt.append(" from t_department_ty a ,(select t.departid,t.departname from t_department_ty t) b ,t_user_department_ty e ,t_user f");
		sqlStmt.append("  where  a.parentdepartid=b.departid(+) and e.userid=f.userid and e.departid=a.departid ");
		sqlStmt.append(" and f.userid='");
		sqlStmt.append(department.getUserid());
		sqlStmt.append("' ");
		sqlStmt.append(" order by a.departid ");
		return (Department)departmentDao.getDepartment_ty(sqlStmt.toString());
	}
	...
	public List getAllChild(Department depart){
		return departmentDao.getAllChild(depart);
	}
...
//上下移动，置顶，置底
	...
	/** @param 流动人口机构树   多条查询 */
	public List getAllDeptList(Department department){
		// TODO: implement
		return departmentDao.getAllDeptList(department);
	}
	/** @param 特性许可证管辖单位查询    多条查询*/
	public List getThGwdwList(Department department) {
	    return departmentDao.getThGwdwList(department);	    
	}
	//获得本级以及本级以下部门信息
	public List getSelfandChild(Department depart){
		return departmentDao.getSelfandChild(depart);
	}
}
