...
public class IckslServiceImpl extends BaseService implements IIckslService {
	...
	public Icksl insertIcksl(Icksl icksl){
		...
		Icksl ickkhExistList = new Icksl();
		...
		if(qyryxx != null)
		{
			...
			if(!isIckslExist)
			{
				...
				if(icksl.getKh()==null||"".equals(icksl.getKh()))
				    ...
				    ickkhExistList = ickslDao.getIckslkhExist(icksl);
			   while(ickkhExistList != null){  //校验ic卡卡号是否存在，存在从新生成
				  icksl.setKh(scbmDao.getScbm(scbmMap));
				  ickkhExistList = ickslDao.getIckslkhExist(icksl);
			   }
	...
	/** @param IC卡受理(t_icksl) 查询单条 ickslid*/
	public Icksl getIckslmax(Icksl icksl){
		// TODO: implement
		return (Icksl)ickslDao.getIckslmax(icksl);
	}
	...
	/** @param 查询具备年检的IC卡  分页查询*/
	public Page getIckNJList(Map map, int pageNo,int pageSize,String sort,String desc)
	{
		return ickslDao.getIckNJList(map, pageNo, pageSize, sort, desc);
	}
	/**ick年检--更新企业人员信息和ick信息*/
	public boolean updateIckNj(Icksl icksl, Qyryxx qyryxx) throws Exception{
		boolean isSuccess = false;
		//1.更新icksl
		this.updateIcksl(icksl);
		Icksl newIcksl = this.ickslDao.getIcksl(icksl);
		if(newIcksl!=null){
			qyryxx.setRyid(newIcksl.getRyid());
		}
		this.qyryxxService.updateQyryxx(qyryxx);
		Qyryxx setQyryxx = new Qyryxx();
		setQyryxx.setCyrybh(qyryxx.getCyrybh());
		setQyryxx.setScbz(0);
		setQyryxx = qyryxxDao.getQyryxx(setQyryxx);
		if(setQyryxx!=null){
			//设置采集点编码(需要改动，从session用户信息中得到)
			javax.servlet.http.HttpSession session = ServletActionContext.getRequest().getSession();
			User user = (User)session.getAttribute(Constants.userKey);
			if(user!=null)
			icksl.setNjr(user.getUseraccount());
			icksl.setNjsj(new Date());
			icksl.setRyid(setQyryxx.getRyid());
			icksl.setHylb(setQyryxx.getHylb());
			icksl.setHylbdm(setQyryxx.getHylbdm());
			ickslDao.updateIckslNJ(icksl);
			isSuccess = true;
		}
		return isSuccess;
	}

	public Icksl getRyAndIckslCjd(Icksl icksl) throws Exception {
		// TODO Auto-generated method stub
		return ickslDao.getRyAndIckslCjd(icksl);
	}
}
