...
public class BjbdtjDaoImpl extends BaseDao implements IBjbdtjDao {
	...
	public String getBjbdtjSql(Bjgz bjgz,Bjxxb bjxxbIn) {
		...
		if(ywbm!=null&&ywbm.toUpperCase().equals("T_DDKHXX")){
			if(txdwxx!=null){
				txdwmc="t_ddqyxx.QYMC SSQYMC"; //特行单位名称
				txdwdz="t_ddqyxx.JYDZ SSQYDZ"; //特行单位地址
				//FromYWBM=FromYWBM.append(" left join t_ddqyxx on t_ddkhxx.ddqyid=t_ddqyxx.ddqyid");
				FromYWBM=FromYWBM.append(" (select * from t_qyjbxx t where t.qyid="+bjxxbIn.getYwbzj2()+") t_ddqyxx");
			}
			if(gxdwxx!=null){
		...
		// 流动人口报警条件
		if(ywbm!=null && ywbm.toUpperCase().equals("T_LDRKJBXX")){
			if(txdwxx != null){
				if(txdwxx.toUpperCase().equals("T_QYJBXX")){
					txdwmc = "T_YGDWXX.QYMC SSQYMC"; //用工单位名称
					txdwdz = "T_YGDWXX.DZXZ SSQYDZ"; //用工单位地址
					FromYWBM.append(" (SELECT T.QYMC QYMC,A.DZXZ DZXZ,T.GXDWBM GXDWBM,T.GXDWMC GXDWMC ");
					FromYWBM.append(" FROM T_QYJBXX T,T_YGDWXX A WHERE T.QYID=A.QYID AND T.QYID=");
					FromYWBM.append(bjxxbIn.getYwbzj2());
					FromYWBM.append(") T_YGDWXX");
				}else if(txdwxx.toUpperCase().equals("T_FWXX")){
					txdwmc="T_FWXX.XM SSQYMC"; //房主姓名
					txdwdz="T_FWXX.DZXZ SSQYDZ"; //房屋地址
					FromYWBM.append(" (SELECT A.XM XM,T.DZXZ DZXZ,T.GXDWBM GXDWBM,T.GXDWMC GXDWMC ");
					FromYWBM.append(" FROM T_FWXX T,T_QYRYXX A WHERE T.RYID=A.RYID AND T.FWID=");
					FromYWBM.append(bjxxbIn.getYwbzj2());
					FromYWBM.append(") T_FWXX");
				}else if(txdwxx.toUpperCase().equals("T_GDXX")){
					txdwmc="T_GDXX.XM SSQYMC"; //工地负责人姓名
					txdwdz="T_GDXX.DZXZ SSQYDZ"; //工地地址
					FromYWBM.append(" (SELECT A.XM XM,T.DZXZ DZXZ,T.GXDWBM GXDWBM,T.GXDWMC GXDWMC ");
					FromYWBM.append(" FROM T_GDXX T,T_QYRYXX A WHERE T.RYID=A.RYID AND T.GDID=");
					FromYWBM.append(bjxxbIn.getYwbzj2());
					FromYWBM.append(") T_GDXX");
				}
			}
			if(gxdwxx != null){
				if(txdwxx.toUpperCase().equals("T_QYJBXX")){
					gxdwmc = "T_YGDWXX.GXDWMC GXDWMC";  //管辖单位名称
					gxdwbm = "T_YGDWXX.GXDWBM GXDWBM";  //管辖单位编码
					y_gxdwmc = "T_YGDWXX.GXDWMC Y_GXDWMC"; //原管辖单位编码
					y_gxdwbm = "T_YGDWXX.GXDWBM Y_GXDWBM"; //原管辖单位编码
				}else if(txdwxx.toUpperCase().equals("T_FWXX")){
					gxdwmc = "T_FWXX.GXDWMC GXDWMC";  //管辖单位名称
					gxdwbm = "T_FWXX.GXDWBM GXDWBM";  //管辖单位编码
					y_gxdwmc = "T_FWXX.GXDWMC Y_GXDWMC"; //原管辖单位编码
					y_gxdwbm = "T_FWXX.GXDWBM Y_GXDWBM"; //原管辖单位编码
				}else if(txdwxx.toUpperCase().equals("T_GDXX")){
					gxdwmc = "T_GDXX.GXDWMC GXDWMC";  //管辖单位名称
					gxdwbm = "T_GDXX.GXDWBM GXDWBM";  //管辖单位编码
					y_gxdwmc = "T_GDXX.GXDWMC Y_GXDWMC"; //原管辖单位编码
					y_gxdwbm = "T_GDXX.GXDWBM Y_GXDWBM"; //原管辖单位编码
				}
			}
			ywlb="'"+hylb+"'";
			ywlbbm="'"+hylbdm+"'";
