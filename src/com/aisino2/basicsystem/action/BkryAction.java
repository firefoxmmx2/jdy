 ...
 * */
	...
	public String queryForExportCX() throws Exception {
		//移除已有的session
		HttpServletRequest request = ServletActionContext.getRequest();
		HttpSession session = request.getSession();
		session.removeAttribute("bkryxxExportResult");
		...
		Page page = bkryService.getListForPage(map, 1, Integer.parseInt(maxRows), sort, dir);
		lBkry = page.getData();
		List lPro = new ArrayList();
		lPro.add("xm");
		lPro.add("gmsfhm");
		...
		Bkry getBkry = new Bkry();
		lBkry = this.getResultCache(getBkry.getClass().getName(), lPro, lBkry, lcache);
		session.setAttribute("bkryxxExportResult", lBkry);
		this.result = "success";
		return "success";
	}
	
	public void getExcelCreateCX() throws Exception {
		HttpServletRequest request = ServletActionContext.getRequest();
		HttpServletResponse response = ServletActionContext.getResponse();
		HttpSession session = request.getSession();
		User user = (User)session.getAttribute(Constants.userKey);
		try {
			...
			List bkryxxList = (List) session.getAttribute("bkryxxExportResult");
			Bjxxb setBkry = new Bjxxb();
			List lColumn = this.getExcelColumn(tabletitle);
			//日期格式化
			for(int i=0;i<bkryxxList.size();i++) {
				SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
				//出生日期
				((Bkry)bkryxxList.get(i)).setCsrqstr(format.format(((Bkry)bkryxxList.get(i)).getCsrq()));
				//布控操作时间
				((Bkry)bkryxxList.get(i)).setBkczsjstr(format.format(((Bkry)bkryxxList.get(i)).getBkczsj()));
				//布控到期时间
				((Bkry)bkryxxList.get(i)).setBkdqsjstr(format.format(((Bkry)bkryxxList.get(i)).getBkdqsj()));
			}
			lResult.add(bbmc);
			lResult.add(lColumn);
			lResult.add(response);
			lResult.add(bkryxxList);
			lResult.add(setBkry);
			this.setExcelCreate("bkry", lResult);
			this.result = "ok";
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public String queryForExportJwCX() throws Exception {
		//移除已有的session
		HttpServletRequest request = ServletActionContext.getRequest();
		HttpSession session = request.getSession();
		session.removeAttribute("bkryxxExportResult");
	...
	public void getExcelCreateJwCX() throws Exception {
		HttpServletRequest request = ServletActionContext.getRequest();
		HttpServletResponse response = ServletActionContext.getResponse();
		HttpSession session = request.getSession();
		User user = (User)session.getAttribute(Constants.userKey);
