...
public class QyjbxxServiceImpl extends BaseService implements IQyjbxxService {
	...
	public Qyjbxx insertQyjbxx(Qyjbxx qyjbxx){
		...
		if(qyjbxx.getQybm()==null||"".equals(qyjbxx.getQybm()))
			...
			qyjbxxqybm =qyjbxxDao.isQyjbxxqybmExist(qyjbxx);
		while(qyjbxxqybm != null){ //校验企业编码是否存在，存在从新生成
			qyjbxx.setQybm(scbmDao.getScbm(scbmMap));
			qyjbxxqybm =qyjbxxDao.isQyjbxxqybmExist(qyjbxx);
		}
		//企业主表增加
		
		//废旧金属业的备案编码需要手工填写，不能自动赋值 modified by mds at 20110302
		String hylbdm = qyjbxx.getHylbdm();
		if(!"E04".equals(hylbdm)&&!"E03".equals(hylbdm)&&!"D".equals(hylbdm))//典当行业的备案编码需要手工填写，不能自动赋值 modified by 何靖 at 201107006 HF-7-1技术支持申请表.doc）
		{
...
//			}
		...
		if(qyjbxx.getYyztdm()==null){
			qyjbxx.setYyztdm("11");
		}
		if(qyjbxx.getYyztmc()==null){
			qyjbxx.setYyztmc("营业");
		}
...
//		}
			...
			if(qyjbxx.getLBafwhtList().size()>0)
			{
				for(int i=0;i<qyjbxx.getLBafwhtList().size();i++)
				{
					...
					if(bafwht.getQdrq()!=null&&bafwht.getHtyxq()!=null)
					{
						...
						c.add(c.MONTH, (int)(bafwht.getHtyxq()*12));
						if(c!=null)
		...
		//企业纸质材料
		if(qyjbxx.getLZzclxxList()!=null&&qyjbxx.getLZzclxxList().size()>0){
			for(int i=0;i<qyjbxx.getLZzclxxList().size();i++){
				Zzcl zzcl = (Zzcl)qyjbxx.getLZzclxxList().get(i);
				zzcl.setQyid(qyNewId);
				zzclDao.insertZzcl(zzcl);
			}
		}
		//保安人员信息添加
	...
	public boolean updateQyjbxx(Qyjbxx qyjbxx) throws Exception{
		...
		if("1".equals(qyjbxx.getActivePageTwo()))
		{
			...
			if(qyjbxx.getLZzclxxList()!=null&&qyjbxx.getLZzclxxList().size()>0){
				for(int i=0;i<qyjbxx.getLZzclxxList().size();i++){
					Zzcl setZzcl = (Zzcl)qyjbxx.getLZzclxxList().get(i);
					setZzcl.setQyid(qyjbxx.getQyid());
					if("new".equals(setZzcl.getCzlx())){
						zzclDao.insertZzcl(setZzcl);
					}
					if("delete".equals(setZzcl.getCzlx())){
						if(setZzcl.getZzclid()!=null){
							zzclDao.deleteZzcl(setZzcl);
						}
					}
				}
			}
	...
	public boolean updateQyjbxxBg(Qyjbxx qyjbxx) throws Exception
	{
		...
		if(qyjbxx != null)
		{
			qyjbxxOld.setQyid(qyjbxx.getQyid());
			qyjbxxOld = qyjbxxDao.getQyjbxx(qyjbxxOld);
			List qybgxxList = new ArrayList();
			//印刷业企业变更时需增加企业变更信息t_qybgxx--------2011-11-21ygl 开始
			if("1".equals(qyjbxxOld.getZt())&&"D".equals(qyjbxxOld.getHylbdm())){
				qyxxChangeJl(qybgxxList,qyjbxxOld,qyjbxx);
				qyjbxx.setQybgxxList(qybgxxList);
			}
			//印刷业企业变更时需增加企业变更信息t_qybgxx--------2011-11-21ygl 结束
			//修改企业信息
			// 当企业名称、经营地址、法人代表姓名、证件类型、证件号码、经营范围（主、兼）、企业类别（主、副）等
			// 重要相关事项发生变更时，需重新进行核查
			if(qyjbxxOld.getQymc()==null)
	...
	public void insertQybgxx(Qyjbxx qyjbxx,Integer qylsid){
		if("变更".equals(qyjbxx.getCzlx())&&("J".equals(qyjbxx.getHylbdm())||"D".equals(qyjbxx.getHylbdm()))){
			String sssf = "hn";//所属省份，默认为hn，需从全局参数中取值
			...
			if(qybgxxList!=null&&qybgxxList.size()>0){
				...
				Map scbmMap = new HashMap();
				if("D".equals(qyjbxx.getHylbdm())){
					scbmMap = addScbmMap("BGXHYS", qyjbxx.getQybm(), sssf, 2);
				}else{
					scbmMap = addScbmMap("BGDJXH", qyjbxx.getQybm(), sssf, 2);
				}
	...
	public void addQybgxx(Qyjbxx qyjbxx,List qybgxxList,Qybgxx qybgxx,String bgqnr,String bghnr,int bglxIndex,boolean isDbnr){
		if(!("变更".equals(qyjbxx.getCzlx())&&("J".equals(qyjbxx.getHylbdm())||"D".equals(qyjbxx.getHylbdm())))){ return; } //不是娱乐业不是变更动作 返回
	...
	//印刷业增加企业变更时，增加企业变更信息详细t_qybgxx  2011-11-21ygl    开始
	public void qyxxChangeJl(List qybgxxList,Qyjbxx qyjbxxOld,Qyjbxx qyjbxxTemp){
		String bgyy = qyjbxxTemp.getBgyy();
		Qybgxx qybgxx = new Qybgxx();
		//特殊的变更类型开始
		String qymc = (qyjbxxOld.getQymc()==null)?"":qyjbxxOld.getQymc();
		String jydz = (qyjbxxOld.getJydz()==null)?"":qyjbxxOld.getJydz();
		String frdb = (qyjbxxOld.getFrdb()==null)?"":qyjbxxOld.getFrdb();
		//String gdxx = (qyjbxxOld.getGdxx()==null)?"":qyjbxxOld.getGdxx();
		String jyfwzy = (qyjbxxOld.getJyfwzy()==null)?"":qyjbxxOld.getJyfwzy();
		String qymcTemp = (qyjbxxTemp.getQymc()==null)?"":qyjbxxTemp.getQymc();
		String jydzTemp = (qyjbxxTemp.getJydz()==null)?"":qyjbxxTemp.getJydz();
		String frdbTemp = (qyjbxxTemp.getFrdb()==null)?"":qyjbxxTemp.getFrdb();
		//String gdxxTemp = (qyjbxxTemp.getGdxx()==null)?"":qyjbxxTemp.getGdxx();
		String jyfwzyTemp = (qyjbxxTemp.getJyfwzy()==null)?"":qyjbxxTemp.getJyfwzy();
		addQybgxx(qybgxxList, qybgxx, qymc, qymcTemp, 0, true); //1 企业名称
		addQybgxx(qybgxxList, qybgxx, jydz, jydzTemp, 1, true); //2 经营地址
		addQybgxx(qybgxxList, qybgxx, frdb, frdbTemp, 2, true); //3 法人
		//addQybgxx(qybgxxList, qybgxx, gdxx, gdxxTemp, 3, true); //4 股东信息
		addQybgxx(qybgxxList, qybgxx, jyfwzy, jyfwzyTemp, 4, true); //5 经营范围-主
		
		String qyzflbm = (qyjbxxOld.getQyzflbm()==null)?"":qyjbxxOld.getQyzflbm();
		String qyfflbm = (qyjbxxOld.getQyfflbm()==null)?"":qyjbxxOld.getQyfflbm();

		String jymj = (qyjbxxOld.getJymj()==null)?"":String.valueOf(qyjbxxOld.getJymj());

		String qyzflbmTemp = (qyjbxxTemp.getQyzflbm()==null)?"":qyjbxxTemp.getQyzflbm();
		String qyfflbmTemp = (qyjbxxTemp.getQyfflbm()==null)?"":qyjbxxTemp.getQyfflbm();

		String jymjTemp = (qyjbxxTemp.getJymj()==null)?"":String.valueOf(qyjbxxTemp.getJymj());

		addQybgxx(qybgxxList, qybgxx, qyzflbm, qyzflbmTemp, 5, true); //6 企业主分类
		addQybgxx(qybgxxList, qybgxx, qyfflbm, qyfflbmTemp, 5, true); //6 企业主分类
		addQybgxx(qybgxxList, qybgxx, jymj, jymjTemp, 7, true); //8 经营面积
	
		//特殊的变更类型结束
		
		//其他的变更类型开始
		String gxdw = (qyjbxxOld.getGxdwbm()==null)?"":qyjbxxOld.getGxdwbm();
		String zzjgbm = (qyjbxxOld.getQyzzjgdm()==null)?"":qyjbxxOld.getQyzzjgdm();
		String lxdh = (qyjbxxOld.getLxdh()==null)?"":qyjbxxOld.getLxdh();
		String yzbm = (qyjbxxOld.getYzbm()==null)?"":qyjbxxOld.getYzbm();
		String chzh = (qyjbxxOld.getChzh()==null)?"":qyjbxxOld.getChzh();
		String gxdwTemp = (qyjbxxTemp.getGxdwbm()==null)?"":qyjbxxTemp.getGxdwbm();
		String zzjgbmTemp = (qyjbxxTemp.getQyzzjgdm()==null)?"":qyjbxxTemp.getQyzzjgdm();
		String lxdhTemp = (qyjbxxTemp.getLxdh()==null)?"":qyjbxxTemp.getLxdh();
		String yzbmTemp = (qyjbxxTemp.getYzbm()==null)?"":qyjbxxTemp.getYzbm();
		String chzhTemp = (qyjbxxTemp.getChzh()==null)?"":qyjbxxTemp.getChzh();
		addQybgxx(qybgxxList, qybgxx, gxdw, gxdwTemp, 8, true); //管辖单位
		addQybgxx(qybgxxList, qybgxx, zzjgbm, zzjgbmTemp, 8, true); //组织机构代码
		addQybgxx(qybgxxList, qybgxx, lxdh, lxdhTemp, 8, true); //联系电话
		addQybgxx(qybgxxList, qybgxx, yzbm, yzbmTemp, 8, true); //邮政编码
		addQybgxx(qybgxxList, qybgxx, chzh, chzhTemp, 8, true); //传真
		
		String jjlxmc = (qyjbxxOld.getJjlxmc()==null)?"":qyjbxxOld.getJjlxmc();
		String zczj = (qyjbxxOld.getZczj()==null)?"":String.valueOf(qyjbxxOld.getZczj());
		String jyfwjy = (qyjbxxOld.getJyfwjy()==null)?"":qyjbxxOld.getJyfwjy();
		String frdbzjlx = (qyjbxxOld.getFrdbzjlb()==null)?"":qyjbxxOld.getFrdbzjlb();
		String jjlxmcTemp = (qyjbxxTemp.getJjlxmc()==null)?"":qyjbxxTemp.getJjlxmc();
		String zczjTemp = (qyjbxxTemp.getZczj()==null)?"":String.valueOf(qyjbxxTemp.getZczj());
		String jyfwjyTemp = (qyjbxxTemp.getJyfwjy()==null)?"":qyjbxxTemp.getJyfwjy();
		String frdbzjlxTemp = (qyjbxxTemp.getFrdbzjlb()==null)?"":qyjbxxTemp.getFrdbzjlb();
		addQybgxx(qybgxxList, qybgxx, jjlxmc, jjlxmcTemp, 8, true); //经济类型名称
		addQybgxx(qybgxxList, qybgxx, zczj, zczjTemp, 8, true); //注册资金
		addQybgxx(qybgxxList, qybgxx, jyfwjy, jyfwjyTemp, 8, true); //经营范围 兼营
		addQybgxx(qybgxxList, qybgxx, frdbzjlx, frdbzjlxTemp, 8, true); //法人代表证件类型
		
		String thbajg = (qyjbxxOld.getThbajg()==null)?"":qyjbxxOld.getThbajg();
		String babh = (qyjbxxOld.getBabh()==null)?"":qyjbxxOld.getBabh();
		String thbajgTemp = (qyjbxxTemp.getThbajg()==null)?"":qyjbxxTemp.getThbajg();
		String babhTemp = (qyjbxxTemp.getBabh()==null)?"":qyjbxxTemp.getBabh();
		addQybgxx(qybgxxList, qybgxx, thbajg, thbajgTemp, 8, true); //特行备案机构
		addQybgxx(qybgxxList, qybgxx, babh, babhTemp, 8, true); //特行备案编号
		
		String frdbzjhm = (qyjbxxOld.getFrdbzjhm()==null)?"":qyjbxxOld.getFrdbzjhm();
		String frdblxfs = (qyjbxxOld.getFrdblxfs()==null)?"":qyjbxxOld.getFrdblxfs();
		String kyrq = (qyjbxxOld.getKyrq()==null)?"":String.valueOf(qyjbxxOld.getKyrq());
		String yysj = (qyjbxxOld.getYysj()==null)?"":qyjbxxOld.getYysj();
		String dwfzr = (qyjbxxOld.getDwfzr()==null)?"":qyjbxxOld.getDwfzr();
		String frdbzjhmTemp = (qyjbxxTemp.getFrdbzjhm()==null)?"":qyjbxxTemp.getFrdbzjhm();
		String frdblxfsTemp = (qyjbxxTemp.getFrdblxfs()==null)?"":qyjbxxTemp.getFrdblxfs();
		String kyrqTemp = (qyjbxxTemp.getKyrq()==null)?"":String.valueOf(qyjbxxTemp.getKyrq());
		String yysjTemp = (qyjbxxTemp.getYysj()==null)?"":qyjbxxTemp.getYysj();
		String dwfzrTemp = (qyjbxxTemp.getDwfzr()==null)?"":qyjbxxTemp.getDwfzr();
		addQybgxx(qybgxxList, qybgxx, frdbzjhm, frdbzjhmTemp, 8, true); //法人代表证件号码
		addQybgxx(qybgxxList, qybgxx, frdblxfs, frdblxfsTemp, 8, true); //法人代表联系方式
		addQybgxx(qybgxxList, qybgxx, kyrq, kyrqTemp, 8, true); //开业日期
		addQybgxx(qybgxxList, qybgxx, yysj, yysjTemp, 8, true); //营业时间
		addQybgxx(qybgxxList, qybgxx, dwfzr, dwfzrTemp, 8, true); //单位负责人
		
		
		
		
		String dwfzrzjhm = (qyjbxxOld.getDwfzrzjhm()==null)?"":qyjbxxOld.getDwfzrzjhm();
		String dwfzrlxfs = (qyjbxxOld.getDwfzrlxfs()==null)?"":qyjbxxOld.getDwfzrlxfs();
		String dwfzrzjhmTemp = (qyjbxxTemp.getDwfzrzjhm()==null)?"":qyjbxxTemp.getDwfzrzjhm();
		String dwfzrlxfsTemp = (qyjbxxTemp.getDwfzrlxfs()==null)?"":qyjbxxTemp.getDwfzrlxfs();
		addQybgxx(qybgxxList, qybgxx, dwfzrzjhm, dwfzrzjhmTemp, 8, true); //单位负责人证件号码
		addQybgxx(qybgxxList, qybgxx, dwfzrlxfs, dwfzrlxfsTemp, 8, true); //单位负责人联系方式

		
		
		String bafzr = (qyjbxxOld.getBafzrxm()==null)?"":qyjbxxOld.getBafzrxm();
		String bz = (qyjbxxOld.getBz()==null)?"":qyjbxxOld.getBz();
		String bafzrTemp = (qyjbxxTemp.getBafzrxm()==null)?"":qyjbxxTemp.getBafzrxm();
		String bzTemp = (qyjbxxTemp.getBz()==null)?"":qyjbxxTemp.getBz();
		String bafzrdh = (qyjbxxOld.getBafzrdh()==null)?"":qyjbxxOld.getBafzrdh();
		String bafzrgmsfhm = (qyjbxxOld.getBafzrgmsfhm()==null)?"":qyjbxxOld.getBafzrgmsfhm();
		String bafzrdhTemp = (qyjbxxTemp.getBafzrdh()==null)?"":qyjbxxTemp.getBafzrdh();
		String bafzrgmsfhmTemp = (qyjbxxTemp.getBafzrgmsfhm()==null)?"":qyjbxxTemp.getBafzrgmsfhm();
		String bwbmdh = (qyjbxxOld.getBwbmdh()==null)?"":qyjbxxOld.getBwbmdh();
		String bwbmdhTemp = (qyjbxxTemp.getBwbmdh()==null)?"":qyjbxxTemp.getBwbmdh();
		addQybgxx(qybgxxList, qybgxx, bafzr, bafzrTemp, 8, true); //保安负责人
		addQybgxx(qybgxxList, qybgxx, bafzrdh, bafzrdhTemp, 8, true); //治安负责人电话
		addQybgxx(qybgxxList, qybgxx, bafzrgmsfhm, bafzrgmsfhmTemp, 8, true); //治安负责人身份证
		addQybgxx(qybgxxList, qybgxx, bwbmdh, bwbmdhTemp, 8, true); //保安部门电话
		addQybgxx(qybgxxList, qybgxx, bz, bzTemp, 8, true); //备注
	}
	public void addQybgxx(List qybgxxList,Qybgxx qybgxx,String bgqnr,String bghnr,int bglxIndex,boolean isDbnr){
		if(isDbnr){ if(bgqnr.equals(bghnr)){ return; } } //如果相同返回
		qybgxx = new Qybgxx();
		qybgxx.setBglxdm(bglxdm[bglxIndex]);
		qybgxx.setBglx(bglx[bglxIndex]);
	...
	/**印刷业企业变更查询专用*/
	public Page getListForPageQybgCx(Map map, int pageNo, int pageSize,
			String sort, String desc) {
		return qyjbxxDao.getListForPageQybgCx(map,pageNo,pageSize,sort,desc);
	}
	//印刷业增加企业变更时，增加企业变更信息详细t_qybgxx  2011-11-21ygl    结束
