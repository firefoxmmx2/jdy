	...
	public Qyryxxhc insertQyryxxhcByRyid(Qyryxxhc qyryxxhc) {
		...
		if(qyryxx!=null)
		{
			...
			if(qyjbxx!=null)
			{
				...
				//取得管辖单位编码
				strGxdwbm = qyjbxx.getGxdwbm();
				qyryxxhcNew.setGxdwdm(strGxdwbm);
				// 判断是否是直辖市
				if(DepartmentUtil.departIsZxs(strGxdwbm)){
					qyryxxhcNew.setDsjgdm(StringUtil.add0(strGxdwbm.substring(0, 2),12));
				}else{
				    qyryxxhcNew.setDsjgdm(StringUtil.add0(strGxdwbm.substring(0, 4),12));
				}
				...
				qyryxxhcNew.setFxjdm(StringUtil.add0(strGxdwbm.substring(0, 6),12));
