   ...
   public String updateUser(Qyryxx qyryxx,Qyryxx qyryxx_bak) throws Exception{
	...
	public boolean updateQyryxxForeignKey(Qyryxx qyryxx,Qyryxx qyryxx_bak)
	{
		...
		if(qyry_zp!=null){
			...
			qyry_zpDao.updateQyrybg_zp(qyry_zp);
			qyryxx.setZpid(qyry_zp.getZpid());
		}
	...
	/** @param 企业人员信息(t_qyryxx) 证件号码查询单条 */
	public Qyryxx getQyryxxzjhm(Qyryxx qyryxx){
		// TODO: implement
		return (Qyryxx)qyryxxDao.getQyryxxzjhm(qyryxx);
	}
    ...
    /** 从业人员注销恢复  时执行查询是否存在同一人员的方法*/
	public List getzxhf_IsQyryxxExist(Qyryxx qyryxx) {
		// TODO Auto-generated method stub
		List qyryxxList  = new ArrayList();
		Map map = new HashMap();
		if(qyryxx != null)
		{
			...
			if("90".equals(qyryxx.getZt())){
				map.put("zt", qyryxx.getZt());
				map.put("lrdwbm", qyryxx.getLrdwbm().substring(0, 5));
			}
			
			String hylbdm = qyryxx.getHylbdm();
			/**娱乐业是判断本行业是否存在相同   */
			if ("J".equals(hylbdm)){
				map.put("hylbdm", hylbdm);
			}
			//如果不是这两个行业的话，则只需要判断同一企业是否存在相同即可
			if (!"J".equals(hylbdm) && !"K".equals(hylbdm)) {
				map.put("qyid", qyryxx.getQyid());
			}
			
			if(qyryxx.getHylbdm()!=null&&!"".equals(qyryxx.getHylbdm())){
				if("Z01".equals(qyryxx.getHylbdm())){//中介机构业只使用执业证编号验证人员是否重复
					if("1".equals(ItemChange.getGlobalpar("zjjgzjzFlg"))){//中介机构执业证flg  1为开启校验
						map=new HashMap();
						map.put("zhiwu", qyryxx.getZhiwu());
						map.put("hylbdm", qyryxx.getHylbdm());
					}
				}
			}
		}
	...
	public List getIsQyryxxExist(Qyryxx qyryxx) {
		...
		if(qyryxx != null)
		{
			...
			if (qyjbxx != null) {
				String hylbdm = qyjbxx.getHylbdm();
				//娱乐业和公共场所中不同企业同一证件号可以添加
				if (!"J".equals(hylbdm) && !"K".equals(hylbdm)) {
					map.put("qyid", qyryxx.getQyid());
				}
				//根据重庆需求修改：要求娱乐业采集点录入从业人员时只检测本行业是否存在相同的证件号码，不检测其他行业已经存在同样证件号的从业人员。
				if ("J".equals(hylbdm)){
					map.put("hylbdm", hylbdm);
				}
			}
