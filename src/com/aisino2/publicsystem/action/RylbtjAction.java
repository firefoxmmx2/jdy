...
public class RylbtjAction extends PageAction{
	...
	public Map queryrylbtjlistmap(Map map ,Rylbtj  setRylbtj) throws Exception{
		...
		if (!DepartmentUtil.departIsZxs(departTemp)){
			if (!setRylbtj.getGxdwdm().equals("")) {
				DeptCode=setRylbtj.getGxdwdm().substring(0, 8);
			} else if(!setRylbtj.getFxjdm().equals("")) {
				DeptCode=setRylbtj.getFxjdm().substring(0, 6);
			} else if(!setRylbtj.getDsjgdm().equals("")) {
				DeptCode=setRylbtj.getDsjgdm().substring(0, 4);
			} else {
			...
			if (!setRylbtj.getGxdwdm().equals("")) {
				...
				DeptCode=setRylbtj.getDsjgdm().substring(0, 2);
			} else {
				DeptCode=departTemp.substring(0, 2);
			}
		}
		...
		if (!DepartmentUtil.departIsZxs(departTemp)){
			if("1".equals(setRylbtj.getGxdwbz())){
				deptlevel=5;
				mindeptlevel=3;
			}else if("1".equals(setRylbtj.getFxjbz())){
				deptlevel=4;
				mindeptlevel=3;
			}else if("1".equals(setRylbtj.getDsjgbz())){
				deptlevel=3;
				mindeptlevel=3;
			}else {
				deptlevel=2;
	...
	public String querylist() throws Exception{
		try {
			...
			Map map = new HashMap();
			queryrylbtjlistmap(map,setRylbtj);
			...
			for(Iterator iter = lRylbtj.iterator(); iter.hasNext();){
				...
				if (oneRylbtj.getGxdwdm()!=null&&!oneRylbtj.getGxdwdm().equals("")){
				} else if (oneRylbtj.getFxjdm()!=null&&!oneRylbtj.getFxjdm().equals("")){
					oneRylbtj.setFxjmc("<a href='#' class='fontbutton' onclick=setDsQuery('"+oneRylbtj.getFxjdm()+"')>"+oneRylbtj.getFxjmc()+"</a>");
				} else if (oneRylbtj.getDsjgdm()!=null&&!oneRylbtj.getDsjgdm().equals("")){
					oneRylbtj.setDsjgmc("<a href='#' class='fontbutton' onclick=setDsQuery('"+oneRylbtj.getDsjgdm()+"')>"+oneRylbtj.getDsjgmc()+"</a>");
				}
	...
	public String querylistx() throws Exception{  //统计查询分县局和派出所使用，避免前台方法重
		try {
			int hjT = 0;
			int glryT = 0;
			int fwryT = 0;
			...
			for(Iterator iter = lRylbtj.iterator(); iter.hasNext();){
				...
				if (oneRylbtj.getGxdwdm()!=null&&!oneRylbtj.getGxdwdm().equals("")){
					...
					oneRylbtj.setFxjmc("<a href='#' class='fontbutton' onclick=setDsQueryx('"+oneRylbtj.getFxjdm()+"')>"+oneRylbtj.getFxjmc()+"</a>");
				} else if (oneRylbtj.getDsjgdm()!=null&&!oneRylbtj.getDsjgdm().equals("")){
					oneRylbtj.setDsjgmc("<a href='#' class='fontbutton' onclick=setDsQueryx('"+oneRylbtj.getDsjgdm()+"')>"+oneRylbtj.getDsjgmc()+"</a>");
				}
