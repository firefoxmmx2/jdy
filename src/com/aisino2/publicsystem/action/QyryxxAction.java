...
public class QyryxxAction extends PageAction{
	...
	public void setTabledataGadGl(List lData) throws Exception{
    	...
    	Qyryxx getQyryxx=new Qyryxx();
    	//this.setDataCustomerCache(getQyryxx, lData, lPro, null, lCol, null);
    	this.setDataCustomer(getQyryxx, lData, lPro, null, lCol);
        this.tabledata=this.getData();
        totalrows=this.getTotalrows();
	}
...
//    	
	...
	public String querylistGadGl() throws Exception{
		...
		try {
			...
			map.put("scbz", "0");
			map.put("allhylbdm", setQyryxx.getAllhylbdm());
...
//	        }
	...
	public String modifyZxhf() throws Exception{
		try {
			...
			setQyryxx=(Qyryxx)this.setClass(setQyryxx, null);
			setQyryxx=qyryxxService.getQyryxx(setQyryxx);
			List jieguo  = new ArrayList();//返回值类型
		    jieguo=qyryxxService.getzxhf_IsQyryxxExist(setQyryxx);
			if(jieguo!=null && jieguo.size()!=0){
				result = "此公民身份号码已另做登记，不能进行恢复操作";
			}else{
		        javax.servlet.http.HttpSession session = ServletActionContext.getRequest().getSession();
				User user = (User)session.getAttribute(Constants.userKey);
				setQyryxx.setCzr(user.getUseraccount());
	...
	public String queryzjhm() throws Exception{
		try {
			Qyryxx setQyryxx=new Qyryxx();
			qyryxx=(Qyryxx)this.setClass(setQyryxx, null);
			qyryxx=qyryxxService.getQyryxxzjhm(qyryxx);
	...
	public String insert() throws Exception{
		try {
			...
			if(isCyryExist!=null && isCyryExist.size()>0){
				if("Z01".equals(setQyryxx.getHylbdm())){//中介机构业根据执业证编号判断重复
					this.result="执业证编号已存在";
				}else{
	 ...
	 * @return
	 */
	public String queryQybmBySession(){
		String result = null;
		HttpSession session = ServletActionContext.getRequest().getSession();
		User user = (User) session.getAttribute(Constants.userKey);
		if(user!=null){
			result = user.getSsdwbm();
		}
		return result;
	}
...
//		lcache.add(lcache2);
	...
	public Map addMap(Qyryxx setQyryxx){
        ...
        if(setQyryxx!=null){
        	...
        	map.put("scbz", setQyryxx.getScbz()); 
        	...
        	if(setQyryxx.getAllhylbdm()!=null&&!"".equals(setQyryxx.getAllhylbdm()))
        	{
        		map.put("allhylbdm", setQyryxx.getAllhylbdm());
        	}
	...
	public String queryForExport() throws Exception{
		...
		try {
			...
			map.put("scbz", "0");
			map.put("allhylbdm", setQyryxx.getAllhylbdm());
	        Page page = qyryxxService.getListGadForPage(map,1, Integer.parseInt(maxRows), sort,dir);
	        lQyryxx = page.getData();
	        dictFanYi(lQyryxx);
	        
	    	session.setAttribute("QyryExportResult", lQyryxx);
			this.result = "success";
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * @throws Exception
	 */
	public String querytjForExport() throws Exception{
		HttpSession session = ServletActionContext.getRequest().getSession();
		session.removeAttribute("QyryExportResult");// 清除session中的导出结果
		String maxRows = QjblUtil.queryQjblVal("exportmaxrows");// 最大导出记录数
		if(maxRows == null || "".equals(maxRows)){
		...
		try {
			...
			map.put("ywm", setQyryxx.getYwm());
			map.put("lrdwbm", lrdwbm);
	        Page page = qyryxxService.getWtjListForPage(map,1, Integer.parseInt(maxRows),sort,dir);
	        lQyryxx = page.getData();
	        dictFanYi(lQyryxx);
	...
	public void exportExcel() throws Exception {
		...
		try {
			...
			List qyryList = (List) session.getAttribute("QyryExportResult");
			Qyryxx setQyryxx=new Qyryxx();
			...
			lResult.add(setQyryxx);
			this.setExcelCreate("Qyryxx", lResult);
			this.result = "ok";
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
